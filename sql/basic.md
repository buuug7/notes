...
数据是以表的形式储存在 RDBMS 中的，表是一系列相关数据的集合，又列跟行组成。
...
where 语句可以使用 AND，OR，和 Not 运算符结合使用。
...
-- any
select name from user where age = any (select age from user where age > 22);
-- equivalent above
select name from user where age = 23 or age = 24;

-- all
select name from user where age = all(select age from user where age > 22);
-- equivalent above
select name from user where age = 23 and age = 24;
```
