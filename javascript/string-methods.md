...
从一个字符串中返回指定的字符,index 的值介于 0 跟字符串长度减一之间的整数.
...
用来判断当前字符串是否是另外一个给定字符串结尾的,返回 true 或者 false.其中第二个参数可选,作为 str 的长度,默认为`str.length`.从结尾算起.
...
```javascript
"red and blue".includes("red"); // true
"red and blue".includes("red", 1); // false
```

### str.indexOf(searchValue [, fromIndex])

返回 searchValue 在 str 中第一次出现的索引值,如果未找到,则返回-1. 其中第二个可选参数默认值为 0,如果 fromIndex 为负值则等同于传入了 0,如果 fromIndex>=str.length,则返回-1.注意 indexOf 区分大小写.

```javascript
"red and blue.".indexOf("and"); // 4
"red and blue.".indexOf("and", 5); // -1
```

### str.lastIndexOf(searchValue [, fromIndex])

返回 searchValue 在 str 中最后出现的位置,如果未找到返回-1.第二个可选参数默认值为 str.length,如果负数则被当做 0 处理,如果大于 str.length 则被当做 str.length.
...
```javascript
"buuug7".match(/[a-z]/g); // ["b", "u", "u", "u", "g"]
"buuug7".match(/[a-z]/); // ["b", index: 0, input: "buuug7", groups: undefined]
"buuug7".match(/f/); // null
```
...
```javascript
"abc_123".replace(/_/, "+"); // "abc+abc"
"abc_123".replace(/(\w+)_(\d+)/, "$2 $1"); // "123 abc"
```
...
### str.splitçç([separate[, limit]])

返回元字符串以分隔符出现位置分隔而成的一个数组,第二个参数 limit 限定分隔的片段数量.
...
```javascript
new String("hello world").valueOf(); // "hello world"
```
...
String.raw`Hi\n!`; // "Hi\n!"
let name = "bob";
String.raw`hello, ${name}`; // "hello, bob"
```
